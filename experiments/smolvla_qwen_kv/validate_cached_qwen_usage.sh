...
if [[ "$MODE" == "b0" ]]; then
  CHECKPOINT=${CHECKPOINT:-outputs/lerobot_smolvla_libero_qwen_kv_b0/checkpoints/last}
  CACHE_ROOT=${CACHE_ROOT:-cache_features_libero_b0_raw_ortho6d}
  REFERENCE_CHECKPOINT=${REFERENCE_CHECKPOINT:-outputs/smolvla_libero_qwen_kv_init_b0}
  OUTPUT=${OUTPUT:-outputs/validation_cached_qwen_usage_b0.json}
else
  CHECKPOINT=${CHECKPOINT:-outputs/lerobot_smolvla_libero_qwen_kv_b2/checkpoints/last}
  CACHE_ROOT=${CACHE_ROOT:-cache_features_libero_b2_native}
  REFERENCE_CHECKPOINT=${REFERENCE_CHECKPOINT:-outputs/smolvla_libero_qwen_kv_init_b2}
