#!/usr/bin/env python
"""Run a resumable 200-rollout LIBERO evaluation for one saved checkpoint.
...
underlying evaluator batches the five fixed initial states of each task.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate one RDT checkpoint on 5 fixed states x 10 tasks x 4 LIBERO suites."
    )
    ...
    parser.add_argument("--env-batch-size", type=int, default=5)
    ...
    parser.add_argument("--wandb-run-name", default="libero-b0-native128-from-oxe20k-full")
...
def main() -> None:
    ...
    expected = len(suites) * 10 * args.episodes_per_task
    result: dict[str, Any] = {
