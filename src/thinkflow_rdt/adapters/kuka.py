...
class KukaStandardizedDataset:
    """
    Kuka TFDS adapter emitting one standardized sample per timestep.

    Output schema matches FractalStandardizedDataset:
      dataset_id: str
      episode_id: str
      step_idx: str
      instruction: str
    ...
    def _load_episodes(
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                "KukaStandardizedDataset requires tensorflow and "
        ...
        for episode_index, raw_episode in enumerate(dataset):
            if max_episodes is not None and seen >= max_episodes:
                break
            if self.only_success and "success" in raw_episode:
                if not bool(raw_episode["success"].numpy()):
                    continue
    ...
    def _convert_episode(self, episode_index: int, steps: list[Any]) -> KukaEpisode:
        instructions: list[str] = []
        images: list[np.ndarray] = []
        ...
        for step in steps:
            ...
            poses.append(
                np.asarray(
                    observation["clip_function_input/base_pose_tool_reached"].numpy(),
                    dtype=np.float32,
                )
            )
